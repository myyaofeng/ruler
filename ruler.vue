...
export default {
     ...
     mounted(){
         ...
         console.log('this.defaultMarkBg',this.defaultMarkBg)
        ...
        this.$nextTick(()=>{
            ...
            var liWidth = Number(liWidthPx.substr(0,liWidthPx.length-2)) 
            //获取每个li的margin-left
            var marginLeftPx = this.getStyle(li,'marginLeft')
            var marginLeft = Number(marginLeftPx.substr(0,marginLeftPx.length-2)) 
            //一个li总占位宽度
            this.liALLWidth = marginLeft+liWidth
            //动态设置尺子的总宽
            this.ul.style.width = this.liALLWidth*(this.maxValue-this.minValue+1)+'px'
     ...
     methods:{
        ...
        getStyle(obj,attr){ 
         ...
         touchmove(ev){          
            ...
            if(this.MarkulLeft >= this.screenW/2){
                ...
                this.ul.style.left = -this.ulW+this.screenW/2+ this.liALLWidth+ 'px'    
            }else{
                //  this.ul.style.left = this.slideX + 'px'
                this.lens =this.screenW/2+this.liALLWidth*(-this.moveBgMark)+this.liALLWidth*this.minValue
                this.ul.style.left = this.lens+'px'
            }
            
         },
         touchend(ev){
            this.$emit('setData',this.moveBgMark)
        //     this.onff =true 
        //     let ulLeftPxone =  this.getStyle(this.ul,'left')
        //     //将尺子的left带px==>数字
        //     this.MarkulLeft = Number(ulLeftPxone.substr(0,ulLeftPxone.length-2))
        //     console.log('MarkulLeft', this.MarkulLeft )
        //      //手指抬起计算偏移多少个li
        //     let endMoveX = (this.MarkulLeft-this.ulmoveLeft)/this.liALLWidth
        //     //对小数个li 四舍五入
        //     let converNum =  endMoveX.toFixed(0)
        ...
        //         this.ul.style.left = this.lens+'px'
        //     }
        //     this.$emit('setData',this.initNum)
        }
     }
}
</script>
